...
const AdminGradeLeaderScreen: React.FC = () => {
    ...
    const [editingGradeLeader, setEditingGradeLeader] = useState<GradeLeaderData | null>(null);
    ...
    const fetchCatechists = async () => {
        try {
            ...
            if (response.data.status === "success") {
                setCatechists(response.data.data.map((user: User) => ({
                    id: user.id,
                    name: user.fullName
                })));
            }
    ...
    const handleAdd = async (values: {
        ...
        try {
            ...
            const isExistingLeader = gradeLeaders.some(
                (leader) =>
                    leader.catechistId === values.catechistId && leader.status === "ACTIVE"
            );
            ...
            if (values.isPrimary === "PRIMARY") {
                const existingPrimary = gradeLeaders.find(
                    (leader) => leader.isPrimary === "PRIMARY" && leader.status === "ACTIVE"
                );
                if (existingPrimary) {
    ...
    const handleEditSubmit = async (values: { isPrimary: string; status: string }) => {
        setEditLoading(true);
        try {
            if (values.isPrimary === "PRIMARY" && editingGradeLeader?.isPrimary !== "PRIMARY") {
                const existingPrimary = gradeLeaders.find(
                    (leader) =>
    ...
    const columns = [
        {
            title: <span className="text-blue-600 font-semibold">Tên giáo lý viên</span>,
    ...
    return (
        <div className="p-6 bg-white rounded-lg shadow-md">
            ...
            <Modal
                title={<div className="text-xl font-bold text-blue-600">Thêm trưởng/phó khối</div>}
                open={isModalVisible}
            ...
            <Modal
                title={<div className="text-xl font-bold text-blue-600">Chỉnh sửa trưởng/phó khối</div>}
            ...
            >
                ...
                >
                    ...
                    >
                        <Radio.Group className="space-x-8">
                            <Radio value="ACTIVE" className="text-base">
                                <span className="text-green-600 font-medium">Hoạt động</span>
                            </Radio>
                            <Radio value="INACTIVE" className="text-base">
            ...
            {selectedYear && selectedGrade ? (
                ...
                <div className="text-center text-gray-500 py-8">
                    <p className="text-lg font-semibold">Vui lòng chọn niên khóa và khối</p>
                    <p className="text-sm">Chọn niên khóa và khối để xem danh sách trưởng khối</p>
                </div>
            )}
        </div>
    );
